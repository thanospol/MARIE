function [Eout,Hout] = SVIE_EM_Coil2Scat_PM_par(Scoord,index,etod,Ct,Ln,Pn,freq,Jc)
%%    Coil to Scatterer PM EM fields generation for the SIE+VIE solver
% _________________________________________________________________________
%
%   Fucntion to generate the POINT MATCHING E and H fields
...
parfor ii = 1:Ne % loop on the elements
    ...
    [Ecoup, Hcoup] = EM_DGF_Triang(Scoord,Ct(:,ii),Ln(:,ii),Pn(:,:,ii),ko);
   
    % add each contribution multiplied by the sign given by etod
    % and multiplied by the current of the DOF
    
    if (idx1)
        Eout = Eout + Jc(idx1)*mult(1)*Ecoup(:,:,1); % first edge E field
        Hout = Hout + Jc(idx1)*mult(1)*Hcoup(:,:,1); % first edge H field
    end
    
    if (idx2)
        Eout = Eout + Jc(idx2)*mult(2)*Ecoup(:,:,2); % second edge E field
        Hout = Hout + Jc(idx2)*mult(2)*Hcoup(:,:,2); % second edge E field
    end
    
    if (idx3)
        Eout = Eout + Jc(idx3)*mult(3)*Ecoup(:,:,3); % third edge E field
