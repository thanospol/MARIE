function [Zbc] = Assembly_SCOUP_PM_par(Scoord,index,etod,Ct,Ln,Pn,freq) 
%%    PM coupling for the SIE+VIE solver (Parallel version)
...
for ii = 1:Ne % loop on the elements
    ...
    if (idx1)
        Zbc(:,:,idx1) = Zbc(:,:,idx1) + mult(1)*Ecoup(:,:,1,ii); % first edge
    end
    
    if (idx2)
        Zbc(:,:,idx2) = Zbc(:,:,idx2) + mult(2)*Ecoup(:,:,2,ii); % second edge
    end
    
    if (idx3)
        Zbc(:,:,idx3) = Zbc(:,:,idx3) + mult(3)*Ecoup(:,:,3,ii); % third edge
    end
