function[Eout] = SVIE_E_Scat2Coil_Q(Scoord,index,etod,node,elem,freq,LEVEL_DVrule,Jb)
...
for ii = 1:Ne % loop on the elements
    ...
    for jj = 1:No % loop on the number of observation points
        ...
        if (idx1)
            Eout(idx1,1) = Eout(idx1,1) + mult(1)*(Jb(jj,:)*GJ1); % first edge
        end
        
        if (idx2)
            Eout(idx2,1) = Eout(idx2,1) + mult(2)*(Jb(jj,:)*GJ2); % second edge
        end
        
        if (idx3)
            Eout(idx3,1) = Eout(idx3,1) + mult(3)*(Jb(jj,:)*GJ3); % third edge
        end
