function [Zbc] = Assembly_SCOUP_Q_par(Scoord,index,etod,node,elem,freq,LEVEL_DVrule) 
...
for ii = 1:Ne % loop on the elements
    ...
    parfor jj = 1:No % loop on the number of observation points
        ...
        Zbc1(jj,:) = Zbc1(jj,:) + mult(1)*GJ1.'; % first edge
        Zbc2(jj,:) = Zbc2(jj,:) + mult(2)*GJ2.'; % second edge
        Zbc3(jj,:) = Zbc3(jj,:) + mult(3)*GJ3.'; % third edge
        
    end
    
    if (idx1)
        Zbc(:,:,idx1) = Zbc(:,:,idx1) + Zbc1; % first edge
    end
    
    if (idx2)
        Zbc(:,:,idx2) = Zbc(:,:,idx2) + Zbc2; % second edge
    end
    
    if (idx3)
        Zbc(:,:,idx3) = Zbc(:,:,idx3) + Zbc3; % third edge
    end
