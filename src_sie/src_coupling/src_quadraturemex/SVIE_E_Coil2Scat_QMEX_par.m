function [Eout] = SVIE_E_Coil2Scat_QMEX_par(Scoord,index,etod,node,elem,freq,LEVEL_DVrule,Jc) 
...
%   Applies openmp accelerated mexed function
%
% _________________________________________________________________________
%
%% Input
...
% call the mex fun: already with openmpi if available
if (exist('ompQuadCoil2Scat', 'file') == 3)
    [Eout] = ompQuadCoil2Scat(R1(:),R2(:),R3(:),NE,RO(:),NO,IDX(:),MULT(:),NC,ko,Np_2D,Z1,Z2,Z3,wp,Jc(:));
else
