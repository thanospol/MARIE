function [Eout] = SVIE_E_Scat2Coil_QMEX_par(Scoord,index,etod,node,elem,freq,LEVEL_DVrule,Jb)
%%    Scatterer to Coil (Parallel) Quadrature E field generation for the SIE+VIE solver
...
%   Applies openmp accelerated QMEX functions
%
% _________________________________________________________________________
%
%% Input
...
% use mexed function: already in openmpi if available
if (exist('ompQuadScat2Coil', 'file') == 3)
    [Eout] = ompQuadScat2Coil(R1(:),R2(:),R3(:),NE,RO(:),NO,IDX(:),MULT(:),NC,ko,Np_2D,Z1,Z2,Z3,wp,Jb(:));
else
