function [fVIEsolver,Gram,tau3,Scoord,dev] = fVIEsolver_Assembly(RHBM,freq,gpu_flag)
%%    Assembly Routine to generate a fVIESolver handle
...
%
%% OUTPUT
%       fVIEsolver - handle to VIE solver
%       Gram - voxel volume
...
if (gpuDeviceCount) && (gpu_flag)  % check GPU existence and user opt
    ...
    if (dev.FreeMemory > memestimated ) % verify memory of GPU and problem size
        ...
        fVIEGPU = @(J)mv_AN(J, fN_gpu, Mr_gpu, Mc_gpu, Gram, transp_flag, idxS3, 1);
...
% define the main function for the matrix vector product
fVIE = @(J)mv_AN(J, fN, Mr, Mc, Gram, transp_flag, idxS3, 0);
fprintf(fid, '\n CPU VIE op handle generated ');

% -------------------------------------------------------------------------
% Define the solver function
% -------------------------------------------------------------------------

fVIEsolver = @(Vrhs,tol,Vini)jvie2solver(Vrhs,fVIE,fVIEGPU,tol,Vini);

fprintf(fid,' \n VIE solver handle generated ');
fprintf(fid, '\n');
fprintf(fid, '\n ----------------------------------------------------------');
