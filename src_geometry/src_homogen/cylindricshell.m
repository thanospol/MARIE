function [idx,epsilon_r,sigma_e,rho,mu_r,sigma_m] = cylindricshell(r,Cnt,inR,exR,Len,e_r,s_e,dens,m_r,s_m)
%%    Define a Cylindric shell, and assigns homogeneous properties
% _________________________________________________________________________
%
%       Defines a Cylindric shell in a given domain.
...
cylinder = @(r)(abs( r(:,:,:,2)-Cnt(2) + 1j*(r(:,:,:,1)-Cnt(1)) ) >= inR) & (abs( r(:,:,:,2)-Cnt(2) + 1j*(r(:,:,:,1)-Cnt(1)) ) <= exR) & ( (r(:,:,:,3)-Cnt(3)) <= ( Len/2 ) ) & ( (r(:,:,:,3)-Cnt(3)) >= (-Len/2));
