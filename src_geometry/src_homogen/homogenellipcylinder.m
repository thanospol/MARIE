function [idx,epsilon_r,sigma_e,rho,mu_r,sigma_m] = homogenellipcylinder(r,Cnt,Rad,Len,Erat,e_r,s_e,dens,m_r,s_m)
...
%
%% OPTIONAL INPUT
%   Erat        ratio of the elipsoid cut
...
if((nargin < 5) || isempty(Erat))
   Erat = [1 1];
else
    if ((sum(find(Erat > 0))) < 2)
        Erat = [1 1];
    end
end
...
cylinder = @(r)( ( ( ( (r(:,:,:,1) - Cnt(1))/Erat(1) ).^2 + ( (r(:,:,:,2) - Cnt(2))/Erat(2) ).^2 ) < Rad^2 ) & ( (r(:,:,:,3)-Cnt(3)) <= ( Len/2 ) ) & ( (r(:,:,:,3)-Cnt(3)) >= (-Len/2)) );
